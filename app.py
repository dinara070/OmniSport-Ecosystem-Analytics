...
def main():
    ...
    with st.sidebar:
        ...
        st.caption("Performance Analytics v7.0")
        st.divider()
 
        # ===== НОВА ФІЧА #3: Завантаження власного файлу =====
...
def render_tactics():
    ...
    with col_right:
        ...
        if mode == "🎯 Пресет тактики":
            ...
            st.info("Клікніть на поле зліва, щоб додати точки активності.")
            # Координати через числові поля (Streamlit не підтримує кліки по canvas напряму)
    ...
    with col_left:
        ...
        def get_preset_points(tactic, spread):
            """Генерує точки розміщення для різних тактичних схем"""
            presets = {
                "4-3-3 (Атака)": [
                    # Воротар
                    (5, 30),
                    # Захист (4)
                    (20, 12), (20, 22), (20, 38), (20, 48),
                    # Середина (3)
                    (42, 18), (45, 30), (42, 42),
                    # Напад (3)
                    (72, 12), (75, 30), (72, 48),
                ...
                "4-4-2 (Баланс)": [
                    (5, 30),
                    (20, 12), (20, 24), (20, 36), (20, 48),
                    (45, 12), (45, 24), (45, 36), (45, 48),
                    (70, 22), (70, 38),
                ],
                "5-3-2 (Захист)": [
                    (5, 30),
                    (18, 8), (18, 20), (18, 30), (18, 40), (18, 52),
                    (40, 18), (40, 30), (40, 42),
                    (65, 22), (65, 38),
                ],
                "3-5-2 (Контроль)": [
            ...
            if tactic in presets:
                ...
                # Додаємо шум до кожної точки та розмножуємо
                pts = []
                for bx, by in base_pts:
                    for _ in range(8):
                        pts.append((
                            np.clip(bx + random.gauss(0, spread / 2), 2, 98),
...
# ==========================================
# 6. СИМУЛЯТОР МАТЧІВ (ОНОВЛЕНО — ЛОГ ПОДІЙ)
# ==========================================
def render_simulator():
    ...
    if st.button("🚀 Почати симуляцію", use_container_width=True):
        ...
        # ===== ГЕНЕРАЦІЯ ЛОГУ ПОДІЙ =====
        log = []
        score1, score2 = 0, 0
 
        # Генеруємо 6–10 подій протягом матчу
        minutes = sorted(random.sample(range(1, 91), random.randint(6, 10)))
 
        event_templates = {
            "гол": [
                "{player} влучив у ворота після сольного проходу!",
                "{player} реалізував штрафний удар!",
                "{player} головою відправив м'яч у сітку!",
                "{player} реалізував пенальті!",
                "{player} забив з-за меж штрафної!",
            ],
            "обіграв": [
                "{player} обіграв суперника на швидкості {speed:.1f} км/год.",
                "{player} вийшов один на один, але не реалізував момент.",
                "{player} зробив ключний дриблінг у центрі поля.",
            ],
            "захист": [
                "{player} перехопив небезпечну передачу.",
                "{player} відбив удар на останніх секундах!",
                "{player} вибив м'яч з-під ніг суперника.",
            ],
            "пас": [
                "{player} зробив ключний пас у штрафну зону.",
                "{player} розпочав небезпечну атаку точним пасом.",
            ],
            "карточка": [
                "{player} отримав жовту картку за грубий фол.",
                "{player} зупинив атаку ціною жовтої картки.",
            ]
        }
 
        for minute in minutes:
            # Вибираємо гравця з урахуванням PER
            ...
            event_text = template.format(
                player=active,
                speed=active_data.get('Швидкість', 25.0)
            )
            ...
            if event_type == "гол":
                if active == p1:
                    ...
                    score2 += 1
                icon = "⚽"
            elif event_type == "захист":
                icon = "🛡️"
            elif event_type == "пас":
                icon = "🎯"
            elif event_type == "карточка":
                icon = "🟨"
            else:
                icon = "💨"
        ...
        # Рахунок (якщо нічия — овертайм через PER)
        if score1 == score2:
...
# ==========================================
# 7. ПРОГНОЗ ТРАВМАТИЗМУ (ОНОВЛЕНО — ML LITE + НАВАНТАЖЕННЯ)
# ==========================================
def render_injury_prediction():
    ...
    # Редагування навантаження прямо на сторінці
    col_edit, col_info = st.columns([1, 2])
    with col_edit:
        st.subheader("📅 Навантаження за тиждень")
        workload = st.number_input(
            "Кількість матчів за останні 7 днів",
            min_value=0, max_value=7,
            value=int(player_data.get('Навантаження_7днів', 0)),
            key="workload_input"
        )
        # Оновлення в базі
        st.session_state.athletes_db.loc[
            st.session_state.athletes_db["Ім'я"] == selected_player, 'Навантаження_7днів'
        ] = workload
 
    with col_info:
        if workload >= 3:
            st.error(f"🚨 **{workload} матчі поспіль** — перевантаження! Ризик травми автоматично підвищено до КРИТИЧНОГО рівня.")
        elif workload == 2:
            st.warning(f"⚠️ **{workload} матчі за тиждень** — підвищене навантаження. Стежте за відновленням.")
        else:
            st.success(f"✅ **{workload} матч(ів) за тиждень** — нормальне навантаження.")
 
    st.divider()
 
    # ===== РОЗРАХУНОК РИЗИКУ (ML LITE) =====
    imbalance = abs(player_data['Сила'] - player_data['Витривалість'])
    base_risk = int((imbalance * 1.5) + (player_data['Швидкість'] * 0.5))
 
    # Фактор навантаження
    if workload >= 3:
        workload_factor = 40  # критичний приріст
    elif workload == 2:
        workload_factor = 20
    else:
        workload_factor = 0
 
    # Фактор дисбалансу витривалості
    if player_data['Витривалість'] < 50:
        stamina_penalty = 15
    else:
        stamina_penalty = 0
    ...
    with c1:
        ...
        st.caption("Формула: дисбаланс сила/витривалість + навантаження + штраф за виснаження")
 
    with c2:
        st.subheader("📊 Фактори ризику")
        factors = {
            "Базовий ризик": base_risk,
            "Навантаження (+)": workload_factor,
            "Виснаження (+)": stamina_penalty,
        }
    ...
    with c3:
        ...
        if workload >= 3:
            st.error("🛑 **Обов'язковий відпочинок 48 год!**")
        st.write(f"**Дієта:** {random.choice(['Більше білка', 'Магній+', 'Гідратація'])}")
        st.write("**Сон:** 8.5 год")
        if player_data['Витривалість'] < 60:
            st.write("**Тренування:** Легке кардіо, без контакту")
        else:
            st.write("**Тренування:** Стандартний план")
...
def render_io():
    ...
    template_df = pd.DataFrame(columns=["Ім'я", "Вид спорту", "Матчі", "Очки",
                                         "Швидкість", "Витривалість", "Сила", "Навантаження_7днів"])
    st.download_button(
        "📄 Завантажити порожній шаблон CSV",
        template_df.to_csv(index=False).encode('utf-8'),
        "template_athletes.csv",
        mime="text/csv"
    )
