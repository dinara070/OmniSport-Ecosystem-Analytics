...
# ==========================================
# 9. 💰 Економіка та Скаутинг
# ==========================================
def render_economy():
    st.title("💰 Економіка та Ефективність (Moneyball)")
    st.markdown("""
    Цей модуль оцінює фінансову ефективність гравців. 
    Ми порівнюємо їхній **рейтинг (PER)** із умовною **ринковою вартістю**, щоб знайти "недооцінених" талантів.
    """)
    
    df = st.session_state.athletes_db.copy()
    
    # Симулюємо ринкову вартість на основі характеристик
    # Вартість = (PER * 100k) + (Молодість бонус)
    if 'Вік' in df.columns:
        df['Ринкова вартість ($)'] = (df['PER (Рейтинг)'] * 125000 - (df['Вік'] * 50000)).clip(lower=50000)
    else:
        df['Ринкова вартість ($)'] = (df['PER (Рейтинг)'] * 80000).clip(lower=50000)

    # Розраховуємо КД (Коефіцієнт Доцільності): PER / Вартість
    df['ROI (Ефективність)'] = (df['PER (Рейтинг)'] / (df['Ринкова вартість ($)'] / 100000)).round(2)

    col1, col2 = st.columns([2, 1])

    with col1:
        st.subheader("📊 Аналіз вартості vs Продуктивності")
        # Scatter chart: Ціна vs Якість
        fig, ax = plt.subplots(figsize=(10, 6))
        for i, row in df.iterrows():
            ax.scatter(row['Ринкова вартість ($)'], row['PER (Рейтинг)'], s=200, alpha=0.6)
            ax.annotate(row["Ім'я"], (row['Ринкова вартість ($)'], row['PER (Рейтинг)']), xytext=(5,5), textcoords='offset points')
        
        ax.set_xlabel("Оціночна вартість ($)")
        ax.set_ylabel("Рейтинг ефективності (PER)")
        ax.grid(True, linestyle='--', alpha=0.5)
        st.pyplot(fig)

    with col2:
        st.subheader("🌟 Кращі за ROI")
        st.caption("Гравці, які дають максимум результату за свою ціну")
        best_value = df.sort_values('ROI (Ефективність)', ascending=False).head(3)
        for i, row in best_value.iterrows():
            st.success(f"**{row['Ім\'я']}**\n\nROI: {row['ROI (Ефективність)']} (Топ-пропозиція)")

    st.divider()
    st.subheader("💵 Повна фінансова таблиця")
    st.dataframe(df[["Ім'я", "PER (Рейтинг)", "Ринкова вартість ($)", "ROI (Ефективність)"]].style.highlight_max(subset=['ROI (Ефективність)'], color='lightgreen'), use_container_width=True, hide_index=True)

# ==========================================
# 10. AI-ТРЕНЕР (ПЛАНИ ТРЕНУВАНЬ)
# ==========================================
